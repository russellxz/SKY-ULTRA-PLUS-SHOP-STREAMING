...
function router(ctx) {
  ...
  r.get("/", (req, res) => {
    ...
    // Sidebar vertical de categorías (estilo lista con estrellas)
    let categoriesHtml = `<aside class="sp-cats">
      <div class="sp-cats-head"><i class="ri-folder-3-line"></i></div>
      ${catList.map(c=>{
    ...
    const html = `<link rel="stylesheet" href="/public/css/store-modern.css?v=5">${err}<div class="sp-page">${heroHtml}${tabsHtml}<div class="sp-layout">${categoriesHtml}<main class="sp-products"><div class="sp-grid">${productsHtml}</div></main></div></div>`;
