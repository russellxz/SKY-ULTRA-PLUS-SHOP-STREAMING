...
function router(ctx) {
  ...
  r.post("/api/create", async (req, res) => {
    try {
      if (!enabled(ctx, "paypal_api_enabled")) return res.status(400).send("PayPal API está deshabilitado.");
      ...
      if (!rsp.ok) { console.error("PayPal create error:", data); return res.status(500).send("No se pudo crear la orden PayPal."); }
      const approve = (data.links || []).find(l => l.rel === "approve")?.href;
      if (!approve) return res.status(500).send("Respuesta PayPal sin link de aprobación.");
  ...
  r.post("/ipn/checkout", (req, res) => {
    if (!enabled(ctx, "paypal_ipn_enabled")) return res.status(400).send("PayPal IPN deshabilitado.");
